...
class DeepClustering(mask_separation_base.MaskSeparationBase):
    ...
    def __init__(self, input_audio_signal,
        ...
        self.masks = []
        self.cutoff = cutoff
        self.pca_before_clustering = pca_before_clustering
    ...
    def deep_clustering(self):
        ...
        if self.pca_before_clustering:
            pca = self.project_arbitrary_embeddings(
                dim_indices=tuple(range(self.num_pca_dimensions)))
            self.embeddings = pca
    ...
    def get_pca(self):
        if not self.pca_been_run:
            self.pca = PCA(self.num_pca_dimensions).fit(self.embeddings)
            self.pca_been_run = True
        return self.pca

    def project_arbitrary_embeddings(self, dim_indices=(0, 1), pca=False):
        """Projects
    ...
    def test_transform(self, pca, data, dim_indices):
        ...
        _components = pca.components_[dim_indices, :].T
