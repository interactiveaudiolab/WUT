...
class SourceSeparation(object):
    ...
    def get_power_spectrogram_json(self, ch):
        power_spectrogram_data = self.get_power_spectrogram_data(ch)
        power_spectrogram_small = np.array(power_spectrogram_data + 80, dtype=np.uint8)
        sig_dur = self.user_input_audio_signal.signal_duration
        max_freq = np.max(self.user_input_audio_signal.freq_vector)

        # power_spectrogram_data_json = nussl.json_serialize_numpy_array(power_spectrogram_small)
        power_spectrogram_data_json = power_spectrogram_small.tolist()
        return jsonify({'power_spectrogram_data': power_spectrogram_data_json,
                        'signal_duration': sig_dur, 'max_frequency': max_freq})
